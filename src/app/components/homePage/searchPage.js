...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const response = await fetchData("GET", "supported-projects")
  ...
  if (loaded) { return (
    <div className="h-full px-16 flex flex-col gap-6">
      <div className="bg-white rounded-xl h-auto p-6 flex flex-row items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Dropdown theme={dropdownTheme} 
            label="Select one or more projects from the dropdown list" dismissOnClick={false} className='max-h-40 overflow-y-auto' disabled={allProjects}>
              {
      ...
      <div className="bg-white rounded-xl h-auto px-6 pt-6 flex justify-between">
        <div className="flex flex-col items-center w-full gap-">
          <div className="flex flex-row justify-between items-center w-full">
            <div className="font-bold text-dark-green self-start">
              SEARCH BY
            </div>
            <div className="flex flex-row gap-4 text-black text-sm items-center">
              Input language: 
              <Dropdown theme={dropdownTheme} 
                label={searchData["inputLanguage"]} dismissOnClick={true} className='max-h-40 overflow-y-auto'>
                  {
                    languages.sort().map(language_option => {
                      return(
                        <DropdownItem id={language_option} label={language_option} onClick={() => changeKey(setSearchData, "inputLanguage", language_option)}> {language_option}
                        </DropdownItem>
                      )
                    })
                  }
              ...
              <div className="p-6 text-sm text-black border rounded-md h-auto w-full flex flex-col gap-6">
                ...
                <div className="w-full flex flex-row gap-6">
                  ...
                  {loading ? <Button color="light" disabled className="rounded-md px-10 shadow-md bg-gray-300 font-semibold w-48"><div className='flex flex-row justify-between gap-3'><Spinner color="success" size="sm"/>Searching...</div></Button> 
                    : <div className='w-48'><Button color="light" className="rounded-md px-10 shadow-md bg-gray-300 hover:bg-gray-300 font-semibold w-48 focus:ring-0" onClick={handleSearch}>Search</Button></div>}
                </div>
                <div className="flex flex-row gap-10">
                  <div className="flex items-center gap-3">
            ...
            <TabItem title="Word proximity">
              <div className="p-6 text-sm text-black border rounded-md h-auto w-full flex flex-col gap-6">
                ...
                <div className="w-full flex flex-row gap-6">
                  ...
                  <div className="w-full flex flex-col gap-3">
                    <TextInput id="search" theme={inputTheme} type="text" placeholder="Type a word or lemma to search for" onChange={(event) => changeKey(setWords, "thirdWord", event.target.value)} value={words["thirdWord"]}/>
                    <div className="flex flex-row gap-3 items-center">
                      <Dropdown theme={dropdownTheme} 
                        label="near" dismissOnClick={true}>
                          {
                            wordProximity.sort().map(option => {
                              return(
      ...
      <div className="w-full h-auto flex flex-row items-start gap-6">
        <div className="bg-white rounded-xl h-auto p-6 flex flex-row items-center gap-6">
          <div className="flex flex-col gap-3 items-center">
            ...
            <div className={`flex flex-col gap-3 ${results && results.length > 0 && !loading ? "" : "hidden"}`}>
            {
